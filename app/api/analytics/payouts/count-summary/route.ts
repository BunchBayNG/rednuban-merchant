...
export async function GET(request: NextRequest) {
  try {
    ...
    const fetchSummary = async (start: string, end: string) => {
      const apiUrl = `https://redcollection.onrender.com/api/v1/analytics/payouts/count-summary?startDate=${start}T00:00:00Z&endDate=${end}T00:00:00Z${merchantOrgId ? `&merchantOrgId=${merchantOrgId}` : ""}`;
