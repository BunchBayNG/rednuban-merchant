...
export async function GET(request: NextRequest) {
  try {
    ...
    const baseApiUrl = "https://redcollection.onrender.com/api/v1/exports/audit-logs/csv";
    ...
    const filename = headers.get("content-disposition")?.match(/filename="(.+)"/)?.[1] || `audit-logs_${new Date().toISOString().split("T")[0]}.csv`;
