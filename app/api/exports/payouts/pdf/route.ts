...
export async function GET(request: NextRequest) {
  try {
    ...
    const baseApiUrl = "https://redcollection.onrender.com/api/v1/exports/payouts/pdf";
    ...
    const filename = headers.get("content-disposition")?.match(/filename="(.+)"/)?.[1] || `payouts_${new Date().toISOString().split("T")[0]}.pdf`;
