...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("Export Excel - API request received:", {
      url: request.url,
      queryParams,
    });
    ...
    console.log("Export Excel - Final params:", {
      merchantOrgIdFromCookie,
      finalMerchantOrgId: merchantOrgId,
    });

    // Build the API URL with all the same parameters as the table uses
    const baseApiUrl = "https://redcollection.onrender.com/api/v1/exports/transactions/excel";
    ...
    console.log("Final Excel API URL:", apiUrl);
      ...
      return NextResponse.json(
        {
          ...
          status: false,
          message: errorData.message || "Failed to export Excel",
          data: null,
        },
        { status: res.status }
      );
    ...
    const filename = headers.get("content-disposition")?.match(/filename="(.+)"/)?.[1] || `transactions_${new Date().toISOString().split("T")[0]}.xlsx`;
