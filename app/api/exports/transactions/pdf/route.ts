...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("Export PDF - API request received:", {
      url: request.url,
      queryParams,
    });
    ...
    console.log("Final PDF API URL:", apiUrl);
    ...
    const res = await fetch(apiUrl, {
      ...
      signal: AbortSignal.timeout(15000), // Slightly longer timeout for PDF generation
    });
      ...
      return NextResponse.json(
        {
          ...
          status: false,
          message: errorData.message || "Failed to export PDF",
          data: null,
        },
        { status: res.status }
      );
    ...
    const filename = headers.get("content-disposition")?.match(/filename="(.+)"/)?.[1] || `transactions_${new Date().toISOString().split("T")[0]}.pdf`;
