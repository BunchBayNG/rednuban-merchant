...
export async function GET(request: NextRequest) {
  try {
    ...
    const baseApiUrl = "https://redcollection.onrender.com/api/v1/exports/vnuban/csv";
    ...
    const filename = headers.get("content-disposition")?.match(/filename="(.+)"/)?.[1] || `vnubans_${new Date().toISOString().split("T")[0]}.csv`;
