...
export async function GET(request: Request) {
  try {
    ...
    // Parameters matching the API documentation
    const queryParams = {
      page: params.page || "0",
      size: params.size || "10",
    ...
    const apiUrl = "https://redcollection.onrender.com/api/v1/api-logs/fetch";
    ...
    if (res.ok && data.status) {
      ...
      return NextResponse.json(
        {
          error: data.detail || data.message || "Failed to fetch API logs data",
          status: res.status,
        },
        { status: res.status }
      );
